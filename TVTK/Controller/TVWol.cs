...
{
    public class TVWol : INotifyPropertyChanged
    {
        ...
        public TVWol() { Status = "-"; }//без конструктора по-умолчанию не сохраняет список в параметрах

        public TVWol(string name, string ip, string mac, string description = "")
        {
            Name = name;
            IP = ip;
            Mac = mac;
        ...
        public async static void CheckStatus(ObservableCollection<TVWol> AllTV) 
        {
            await Task.Run(() =>
            {
                string result = "-";
